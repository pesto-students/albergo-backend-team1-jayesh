...
import { defaultHotelProjectile, defaultUserProjectile } from "../utils/helperFunctions";
...
router.get("/me", [verifyToken, validateToken, allowRoleUser, checkTokenRoleDB, checkTokenUuidDB], async (req: Request, res: Response) => {
    ...
    try {
        const userDoc = await UserModel.findOne({ uuid: parsedToken.uuid }, defaultUserProjectile);
